...
import { Monitor, LogOut, Shield, Users, Smartphone, AlertTriangle } from "lucide-react";
...
export default function SessionManagementPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setSessions(Array.isArray(sessRes.data?.data) ? sessRes.data.data : []);
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex gap-3 text-xs text-muted-foreground mt-0.5">
                                                        <span>Login: {session.loginAt ? new Date(session.loginAt).toLocaleString("en-IN") : "—"}</span>
                                                        <span>Last active: {session.lastActivity ? new Date(session.lastActivity).toLocaleString("en-IN") : "—"}</span>
                                                    </div>
                                                </div>
                                            </div>
